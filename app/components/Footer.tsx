...
export default function Footer() {
  ...
  // Hide footer on reader pages (immersive reading)
  if (pathname?.startsWith("/reader")) return null;

  const year = new Date().getFullYear();

  // Handle Reddit share - opens Reddit post creation with pre-filled info
  const handleRedditShare = () => {
    const shareUrl = typeof window !== 'undefined' ? window.location.origin : '';
    const title = 'Check out Manhua Rush - Free Fan Translations!';
    const redditShareUrl = `https://reddit.com/submit?url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(title)}`;
    ...
    <footer className="footer-root">
      ...
      <div className="footer-inner">
        ...
        <div className="footer-brand">
          <Link href="/" className="footer-logo-link" aria-label="ManhuaRush home">
            <img
              src={encodeURI("/logo/Fierce dragon with purple lightning.webp")}
              alt="Dragon"
              className="footer-dragon"
            />
            <img
          ...
          <div className="footer-socials">
            ...
            <button
              ...
              title="Share Manhua Rush on Reddit"
            >
              {/* Reddit icon */}
              <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18">
      ...
      <div className="footer-bottom">
        <p className="footer-copy">
          © {year} <span className="footer-copy-brand">ManhuaRush</span>. Fan translations only — all original works belong to their respective creators.
        </p>
        <p className="footer-made-with">
          Made with <span className="footer-heart">♥</span> for fans, by fans.
        </p>
      </div>
    </footer>
  );
